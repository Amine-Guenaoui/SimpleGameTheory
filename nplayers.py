...
def str_dom_str(comb, nbr_stra, n_players):
    ...
    for player in range(n_players):
        ...
        str1 = getStrategyValues(player, strategy, comb)
...
def getStrictWeakestStrategy(tempComb, player, nbr_stra):
    ...
    strategy = 0
    # current strategy values
    str1 = getStrategyValues(player, strategy, tempComb)
    if len(str1) > 0:
        strategy2 = strategy+1
        while strategy2 < nbr_stra[player]:
...
def get_min_str(player, strp, player_comb):
...
def get_min_all_str(player, player_comb, nbr_stra):
    all_str_min = []
    for strp in range(nbr_stra[player]):
        str_min = get_min_str(player, strp, player_comb)
...
def prof_pareto(comb, nbr_stra, n_players):
...
def niv_sec(comb, nbr_stra, n_players):
    ...
    while player < n_players:
        player_comb = get_val_all_str(player, comb)
        print(player_comb)
        str_min = get_min_all_str(player, player_comb, nbr_stra)
        print(str_min)
        # equil.append(player)
...
def init():
    ...
    n_players = int(input("entrer le nombre des joueurs"))
    ...
    for i in range(n_players):
        ...
        nbr_stra.append(
            int(input("nombre strategies pour joureur " + str((i+1)))))
    ...
    comb = []
    # comb contient tous les valeurs de la matrice avec leurs indices which we will use for our treatment
    showIndexes(matrix, comb)
...
while not leave:
   ...
   #comb,nbr_stra,n_players = init()
    while x != "n" or x != "non":
        ...
        if choice == 6:
            prof_pareto(comb, nbr_stra, n_players)
        if choice == 7:
            niv_sec(comb, nbr_stra, n_players)
